...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
InitVDP:
		move.w	#$8134,r_VDP_Reg_1.w		; Save VDP register 1 in RAM
		move.w	#$8AFF,r_HInt_Reg.w		; Save H-INT counter register in RAM
...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
LoadPalette:
		lea	r_Palette.w,a1			; Main palette buffer
...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
LoadTargetPal:
		lea	r_Dest_Pal.w,a1			; Target palette buffer
		bra.s	LoadPalToBuf			; Load the palette
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
LoadWaterPal:
		lea	r_Water_Pal.w,a1		; Main water palette buffer
		bra.s	LoadPalToBuf			; Load the palette
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
...
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
LoadTargetWaterPal:
		lea	r_Dest_UW_Pal.w,a1		; Target water palette buffer
; ---------------------------------------------------------------------------------------------------------------------------------------------------------
...
.FadeLoop:
		rept	2
			move.b	#vFade,r_VINT_Rout.w	; Set V-INT routine
...
.Loop:
		...
		clrRAM	r_Spr_Input			; Clear sprite inputs
...
DrawSpr_BndChk_Loop:
		...
		move.w	d2,(a6)+			; Store in sprite table
		subq.w	#1,d7				; Decrement sprite count
		dbmi	d4,DrawSpr_BndChk_Loop		; Loop if there are still enough sprites left
		rts

.Next_XOffScr:
		subq.w	#6,a6				; Go back to the start of the current sprite entry
		dbf	d4,DrawSpr_BndChk_Loop		; Loop if there are still enough sprites left
		rts
...
.Loop:
		...
		move.w	(a1)+,d2			; Get X offset
		neg.w	d2				; Negate it
		move.b	DrwSpr_BndChk_XFlips(pc,d6.w),d6; Get the X offset to apply
		sub.w	d6,d2				; Subtract the new X offset
		add.w	d0,d2				; Add onto X position
		cmpi.w	#-32+128,d2			; Is it left of the screen?
		bls.s	.Next_XOffScr			; If so, branch
...
DrawSprite_BndChk_FlipXY:
		...
		neg.w	d2				; Negate it
		move.b	(a1),d6				; Get sprite sizes
		move.b	DrwSpr_BndChk_YFlips(pc,d6.w),d6; Get the Y offset to apply
		sub.w	d6,d2				; Subtract from the Y offset
		add.w	d1,d2				; Add onto Y position
		cmpi.w	#-32+128,d2			; Is it above the screen?
		bls.s	.Next_YOffScr			; If so, branch
